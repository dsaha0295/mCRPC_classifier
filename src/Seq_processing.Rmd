---
title: "Create consensus set of RNA, deep RNA, ATAC peaks, and DNAm HMR"
...
counts <- read.table("/Users/ds/Desktop/projects/data/rna/mCRPC_RNA_counts_coord.txt", header = T)
...
rna_cpm <- cpm(y = as.matrix(counts[keep,])) %>% data.frame(Coordinates = row.names(.)) %>% dplyr::select(Coordinates, everything())



#Save to disk
write.table(rna_cpm, file= "/Users/ds/Desktop/projects/data/rna/mCRPC_RNA_CPM_coord.txt", quote = F, sep = "\t", row.names = F, col.names = T)
...
dba_res <- readRDS(file = "/Users/ds/Desktop/scratch/ATAC/Data/ATAC_mCRPC_DBA_results.rds") 
...
dnam <- read.csv("/Users/ds/Desktop/scratch/txt/rHMR_mCRPC.csv", header = T, skip = 1)

#Subset for HMRs in 50% or more out of 100 samples
dnam <- dnam %>% filter(X..samples >= 50)
...
GRanges(dnam) %>% saveRDS("/Users/ds/Desktop/scratch/SV_association/HMR/rHMR.rds")

#write.table(dnam, file = "/Users/ds/Desktop/scratch/SV_association/HMR/rHMR.bed", quote = F, sep = "\t", row.names = F, col.names = F)
